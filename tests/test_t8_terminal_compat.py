...
def _gen_py(charset: str, w: int, h: int) -> str:
    img = Image.new("RGB", (w, h), (60, 120, 180))
    img.save("_t8tmp.png")
    try:
        seq = convert("_t8tmp.png", charset, w, h)
    finally:
        os.remove("_t8tmp.png")
    return render(seq, "python")


def test_py_contains_ansi_blocks_codes():
    """blocks 风格 .py 应含 ANSI 38;2;（前景）和 48;2;（背景）转义码。"""
    src = _gen_py("blocks", 16, 8)
    assert "38;2;" in src
    assert "48;2;" in src


def test_py_has_enable_windows_ansi_call():
...
def test_py_ansi_enable_failure_fallback(monkeypatch):
