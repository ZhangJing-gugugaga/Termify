...
**Q: 下载的文件多大？**
A: 取决于尺寸和帧数（播放器骨架本身约 20KB）：
- 80×24 ascii 静态图单帧 → ~23 KB（实测）
- 120×36 blocks 24 帧 → 数百 KB
- 200×60 blocks 100 帧 → 数 MB
...
| `POST` | `/api/generate` | 打包指定字符集+格式（`python`/`html`/`mp4`），返回 `download_url`。可选 `color`（同 preview，产物文件名带 `_src`/`_src256` 段）、`fg`/`bg`（单色）。`mp4` 为同步编码（限 12 次/分钟，2 路并发），需服务器安装 ffmpeg |
